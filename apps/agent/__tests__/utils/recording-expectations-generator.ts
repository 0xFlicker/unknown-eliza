...
export class RecordingExpectationsGenerator {
  ...
  generateExpectationUpdates(testSuite: string, testName: string): string[] {
    ...
    try {
      ...
      recording.recordings.forEach((call: any, index: number) => {
        ...
        suggestions.push(
          `// Call ${index + 1}: ${call.agentId} - ${call.modelType}`,
        );
        suggestions.push(`// Prompt: ${call.prompt}`);
        suggestions.push(
          `expect(response${index + 1}).toContain("${this.escapeString(responseText.substring(0, 50))}");`,
        );
        suggestions.push("");
      });
  ...
  generateExpectationsFile(
    outputPath: string = "./test-expectations.ts",
  ): void {
...
`;
  ...
  detectResponseDrift(
    ...
    if (!existsSync(recordingPath)) {
      return [
        `No recording found to compare against for ${testSuite}::${testName}`,
      ];
    }
      ...
      recording.recordings.forEach((call: any, index: number) => {
        if (index < actualResponses.length) {
          ...
          if (recordedResponse !== actualResponse) {
            driftReport.push(
              `📊 Response drift detected for call ${index + 1}:`,
            );
            driftReport.push(
              `   Recorded: ${recordedResponse.substring(0, 100)}...`,
            );
            driftReport.push(
              `   Actual:   ${actualResponse.substring(0, 100)}...`,
            );
            driftReport.push("");
          }
        }
   ...
   */
  private getAllRecordings(): Array<{
    testSuite: string;
    testName: string;
    filePath: string;
  }> {
