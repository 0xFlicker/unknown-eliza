...
import {
  PlayerRelationship,
  PlayerStatement,
} from "@elizaos/plugin-social-strategy";
...
export class ScenarioVerifier {
  ...
  async verifyEntity(
    ...
    if (expected.platformIdentities) {
      ...
      for (const expectedIdentity of expected.platformIdentities) {
        const found = platformIdentities.find(
          (pi) =>
            pi.platform === expectedIdentity.platform &&
            pi.handle === expectedIdentity.handle
        );
    ...
    logger.info("[ScenarioVerifier] Entity verified", {
      entityId,
      names: entity.names,
    });
  }
  ...
  async verifyRelationship(
    ...
    if (expected.exists && !relationship) {
      throw new Error(
        `Expected relationship between ${entityA} and ${entityB} not found`
      );
    }

    if (!expected.exists && relationship) {
      throw new Error(
        `Unexpected relationship found between ${entityA} and ${entityB}`
      );
    }
    ...
    if (
      expected.type &&
      relationship.metadata.relationshipType !== expected.type
    ) {
      throw new Error(
        `Relationship type mismatch: expected ${expected.type}, got ${relationship.metadata.relationshipType}`
      );
    }
    ...
    if (
      expected.sentiment &&
      relationship.metadata?.sentiment !== expected.sentiment
    ) {
      throw new Error(
    ...
    if (expected.minStrength !== undefined) {
      const strength =
        ...
        (relationship.metadata?.strength as number) ||
        0;

      if (strength < expected.minStrength) {
        throw new Error(
          `Relationship strength ${strength} below minimum ${expected.minStrength}`
        );
      }
    }
    ...
    logger.info("[ScenarioVerifier] Relationship verified", {
      sourceEntityId: relationship.sourceEntityId,
      targetEntityId: relationship.targetEntityId,
      type: relationship.metadata.relationshipType,
    });
  }
  ...
  async verifyDispute(
    ...
    if (expected.count !== undefined && disputes.length !== expected.count) {
      throw new Error(
        `Dispute count mismatch: expected ${expected.count}, got ${disputes.length}`
      );
    }

    if (expected.disputedField && disputes.length > 0) {
      const hasField = disputes.some(
        (d) => d.data.disputedField === expected.disputedField
      );
      if (!hasField) {
        throw new Error(
          `No dispute found for field: ${expected.disputedField}`
        );
      }
    }
    ...
    logger.info("[ScenarioVerifier] Disputes verified", {
      entityId,
    ...
    if (expected.sharingRestrictions) {
      ...
      for (const restriction of expected.sharingRestrictions) {
        if (!restrictions.includes(restriction)) {
          throw new Error(
            `Missing expected sharing restriction: ${restriction}`
          );
        }
      }
    }

    logger.info("[ScenarioVerifier] Privacy verified", {
      entityId,
      hasPrivateData,
    });
  }
  ...
  async verifyComponent(
    ...
    const exists =
      components.filter((c) => c.type === componentType).length > 0;
    ...
    logger.info("[ScenarioVerifier] Component verified", {
      entityId,
      componentType,
      exists,
    });
  ...
  async verifyMentionedPerson(
    ...
    const components = await this.runtime.getComponents(mentionedBy);
    const memories = components.filter(
    ...
    for (const memory of memories) {
      if (memory.entityId) {
        console.log(`Found memory: ${memory.entityId}`);
    ...
    logger.info("[ScenarioVerifier] Mentioned person verified", {
      personName,
      mentionedBy,
    });
  }
