...
export const MemoizedMessageContent = React.memo(
  MessageContent,
  (prevProps, nextProps) => {
...
export default function Chat({
  ...
  const { data: agentDmChannels = [], isLoading: isLoadingAgentDmChannels } =
    useDmChannelsForAgent(chatType === ChannelType.DM ? contextId : undefined);
    ...
    confirm(
      ...
      async () => {
        ...
        try {
          ...
          toast({
            title: "Chat Deleted",
            description: `"${channelToDelete.name}" was deleted.`,
          });
          ...
          toast({
            title: "Error",
            description:
              "Could not delete chat. The server might not support this action yet.",
            variant: "destructive",
          });
        }
      },
  ...
  useEffect(() => {
    if (chatType === ChannelType.DM && targetAgentData?.id) {
      ...
      chatType !== ChannelType.DM &&
      chatState.currentDmChannelId !== null
    ) {
  ...
  useEffect(() => {
    const isInitialLoadWithMessages =
  ...
  const updateChatTitle = async () => {
    const timestampChatNameRegex =
      /^Chat - [A-Z][a-z]{2} \d{1,2}, \d{2}:\d{2}:\d{2}$/;
  ...
  const handleSendMessage = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (currentSelectedFiles.length > 0) {
        const { uploaded, failed, blobUrls } =
          await uploadFiles(currentSelectedFiles);
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      onClick={() =>
                        updateChatState({ showProfileOverlay: true })
                      }
                    >
              ...
              <div className="flex items-center gap-1">
                ...
                <SplitButton
                  mainAction={{
                    ...
                    disabled:
                      chatState.isCreatingDM || isLoadingAgentDmChannels,
                  }}
                  actions={[
                    {
          ...
          {groupAgents.length > 0 && (
            <div className="flex items-center gap-2 p-2 bg-card rounded-lg border overflow-x-auto">
              ...
              <div className="flex gap-2 min-w-0">
                <Button
                  variant={
                    !chatState.selectedGroupAgentId ? "default" : "ghost"
                  }
                  size="sm"
                  onClick={() =>
                    updateChatState({ selectedGroupAgentId: null })
                  }
                ...
                {groupAgents.map((agent) => (
                  <Button
                    key={agent?.id}
                    variant={
                      chatState.selectedGroupAgentId === agent?.id
                        ? "default"
                        : "ghost"
                    }
                    size="sm"
      ...
      <div className="h-full flex flex-col relative overflow-hidden">
        ...
        {isFloatingMode ? (
          ...
          >
            <ResizablePanel
              defaultSize={
                showSidebar && !chatState.isMobile ? mainPanelSize : 100
              }
