          ...
          <TooltipContent side="top" align="center" className="max-w-xs">
            <div className="flex flex-col gap-2">
              <div
                className={cn(
                  "font-semibold",
                  isUnauthorized ? "text-yellow-500" : "text-red-500",
                )}
              >
                {getErrorMessage()}
              </div>
              <p className="text-xs">
                Please ensure the Eliza server is running and accessible.
              </p>
              {!isUnauthorized && (
                <p className="text-xs">
                  Try refreshing the connection or check server logs.
                </p>
              )}
              {isUnauthorized && (
                <p className="text-xs">
                  Check the X-API-KEY configured in your client or the
                  ELIZA_SERVER_AUTH_TOKEN on the server.
                </p>
              )}
            </div>
          </TooltipContent>
