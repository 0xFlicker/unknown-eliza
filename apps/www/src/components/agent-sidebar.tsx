...
export function AgentSidebar({
  ...
  const { currentTab: detailsTab, setTab: setDetailsTab } =
    useAgentTabState(agentId);
  const { data: panelsResponse, isLoading: isLoadingPanels } = useAgentPanels(
    agentId!,
    {
      enabled: !!agentId,
    },
  ...
  const allTabs: { value: TabValue; label: string; icon: JSX.Element }[] =
    useMemo(() => {
      const fixedTabs: {
        value: FixedTabValue;
        label: string;
        ...
        {isLoadingPanels && (
          <>
            {[...Array(2)].map((_, i) => (
              <Skeleton
                key={`skel-tab-${i}`}
                className="h-9 w-full rounded-md flex-shrink-0"
              />
            ))}
          </>
        )}
