...
export default function MemoryGraph({
  ...
  const data = useMemo(() => {
    ...
    const embeddings = memoriesWithEmbeddings.map(
      (m) => m.embedding as number[],
    );
    ...
    if (sortedMemories.length > 1) {
      for (let i = 0; i < sortedMemories.length - 1; i++) {
        const sourceNode = nodes.find(
          (n) => n.memory.id === sortedMemories[i].id,
        );
        const targetNode = nodes.find(
          (n) => n.memory.id === sortedMemories[i + 1].id,
        );
  ...
  return (
    <div
      ref={graphRef}
      className="w-full h-full bg-muted/10 rounded-lg relative"
    >
      {/* Legend */}
        ...
        data.nodes.length > 0 && (
          <ForceGraph2D
            ...
            nodeCanvasObject={(node: any, ctx, globalScale) => {
              const label =
                node.memory?.entityId === node.memory?.agentId ? "A" : "U";
