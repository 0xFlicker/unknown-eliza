...
export default function AgentRoute() {
  ...
  useEffect(() => {
    clientLogger.info("[AgentRoute] Component mounted/updated", {
      agentId,
      channelId,
    });
    return () => {
      clientLogger.info("[AgentRoute] Component unmounted", {
        agentId,
        channelId,
      });
    };
  ...
  const { data: agentDataResponse, isLoading: isLoadingAgent } =
    useAgent(agentId);
        ...
        status:
          agentDataResponse.data.status === "active"
            ...
            : agentDataResponse.data.status === "inactive"
              ...
              : CoreAgentStatusEnum.INACTIVE,
        username:
          agentDataResponse.data.username ||
          agentDataResponse.data.name ||
          "Unknown",
  ...
  if (!isActive) {
    clientLogger.info(
      "[AgentRoute] Agent is not active, rendering inactive state UI",
      {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full w-full p-8 text-center">
        <h2 className="text-2xl font-semibold mb-4">
          {agentFromHook.name} is not active.
        </h2>
        <p className="text-muted-foreground mb-6">
          Press the button below to start this agent.
        </p>
