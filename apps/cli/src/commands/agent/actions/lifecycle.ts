    ...
    if (options.character) {
      try {
        ...
        if (parsedPaths.length === 0) {
          throw new Error(
            `Invalid character specification: ${options.character}`
          );
        }
              ...
              if (startResponse.ok) {
                ...
                throw new Error(
                  errorData?.error?.message ||
                    `Failed to start agent: ${startResponse.statusText}`
                );
              }
            } else {
              throw new Error(
                `Failed to create/find character: ${characterPath}`
              );
            }
          } catch (error) {
            failedAgents.push(
        ...
        if (failedAgents.length > 0) {
          console.error(
            `\x1b[31m[✗] Failed to start ${failedAgents.length} agent(s):\x1b[0m`
          );
  ...
  try {
    ...
    if (opts.all) {
      ...
      try {
        ...
        logger.info("All ElizaOS agents stopped successfully!");
      } catch (error) {
        logger.error(
    ...
    for (const characterName of parsedPaths) {
      try {
        ...
        const response = await fetch(`${baseUrl}/${resolvedAgentId}/stop`, {
          method: "POST",
        });
    ...
    if (failedAgents.length > 0) {
      console.error(
        `\x1b[31m[✗] Failed to stop ${failedAgents.length} agent(s):\x1b[0m`
      );
      failedAgents.forEach((error) => console.error(`  ${error}`));
    }
  } catch (error) {
