...
describe("Logger", () => {
  ...
  describe("Log Filtering", () => {
    it("should filter service registration logs in non-debug mode", () => {
      ...
      filteredMessages.forEach((msg) => {
        expect(() =>
          customLogger.info({ agentName: "test", agentId: "123" }, msg),
        ).not.toThrow();
      });
    });
  ...
  describe("Hook Methods", () => {
    it("should handle various log input formats", () => {
      ...
      expect(() =>
        customLogger.info({ key: "value" }, "With object"),
      ).not.toThrow();
      expect(() =>
        customLogger.info("Multiple", "string", "parts"),
      ).not.toThrow();
      expect(() =>
        customLogger.error(new Error("Test"), "With error"),
      ).not.toThrow();
    });
    ...
    it("should handle mixed arguments with errors", () => {
      ...
      expect(() =>
        customLogger.error("Message", error, { extra: "data" }),
      ).not.toThrow();
    });
  });
  ...
  describe("Comprehensive Hook Method Coverage", () => {
    ...
    it("should handle object with multiple string arguments", () => {
      ...
      expect(() =>
        customLogger.info({ userId: "123" }, "User", "logged", "in"),
      ).not.toThrow();
    });
    ...
    it("should handle string with error in rest arguments", () => {
      ...
      expect(() =>
        customLogger.error("Multiple errors:", error1, error2),
      ).not.toThrow();
    });
    ...
    it("should handle mixed object and string arguments", () => {
      ...
      expect(() =>
        customLogger.info("Status:", { active: true }, "for user"),
      ).not.toThrow();
    });
    ...
    it("should handle non-string, non-error objects in arguments", () => {
      ...
      expect(() =>
        customLogger.info("Data:", { complex: { nested: true } }, [
          "array",
          "data",
        ]),
      ).not.toThrow();
    ...
    it("should handle Sentry exception capture for errors", () => {
      ...
      expect(() => customLogger.error(error)).not.toThrow();
      expect(() =>
        customLogger.error("Message with error:", error),
      ).not.toThrow();
    ...
    it("should handle all argument being non-string objects", () => {
      ...
      expect(() =>
        customLogger.info({ a: 1, b: 2, c: 3 }, "Combined object log"),
      ).not.toThrow();
      expect(() =>
        customLogger.info("Objects:", { a: 1 }, { b: 2 }),
      ).not.toThrow();
    });
  ...
  describe("InMemoryDestination Coverage", () => {
    it("should handle non-JSON string data in write method", () => {
      ...
      expect(() =>
        customLogger.info("Simple text that might not be JSON"),
      ).not.toThrow();
    });
  ...
  describe("Additional Custom Level Tests", () => {
    it("should use custom log levels", () => {
      ...
      expect(() =>
        (customLogger as any).log("Custom log message"),
      ).not.toThrow();
      expect(() =>
        (customLogger as any).progress("Progress update"),
      ).not.toThrow();
      expect(() =>
        (customLogger as any).success("Operation successful"),
      ).not.toThrow();
    });
  });
  ...
  describe("Custom Prettifier Edge Cases", () => {
    it("should handle undefined level in prettifier", () => {
      ...
      expect(() =>
        customLogger.info("Test with potential undefined level"),
      ).not.toThrow();
    });
    ...
    it("should handle null level in prettifier", () => {
      ...
      expect(() =>
        (customLogger as any).child({ level: null }).info("Test"),
      ).not.toThrow();
    });
    ...
    it("should handle object level data in prettifier", () => {
      ...
      expect(() =>
        customLogger.child({ level: 30 }).info("Test with numeric level"),
      ).not.toThrow();
    });
  });
});
