...
export class DrizzleSchemaIntrospector {
  parseTableDefinition(table: any, exportKey?: string): TableDefinition {
    ...
    if (!compositePrimaryKey && KNOWN_COMPOSITE_PRIMARY_KEYS[tableName]) {
      ...
      logger.debug(
        `[INTROSPECTOR] Using known composite primary key for ${tableName}`,
      );
    }
  ...
  private getTableName(table: any, exportKey?: string): string {
    ...
    if (!table) {
      logger.debug(
        `[INTROSPECTOR] No table provided, using fallback: unknown_table`,
      );
    ...
    for (const symbol of symbols) {
      if (
        symbol.description &&
        symbol.description.includes("drizzle:OriginalName")
      ) {
        const tableName = table[symbol];
        if (typeof tableName === "string") {
  ...
  private parseColumnsFallback(table: any): ColumnDefinition[] {
    ...
    for (const [key, value] of Object.entries(table)) {
      ...
      if (col && (col.columnType || col.config || col.dataType)) {
        ...
        columns.push({
          name: columnName,
          type: this.mapDrizzleColumnType(
            col.columnType || "unknown",
            config,
            columnName,
          ),
  ...
  private parseForeignKeys(table: any): ForeignKeyDefinition[] {
    ...
    const symbols = Object.getOwnPropertySymbols(table);
    const fkSymbol = symbols.find((s) =>
      s.description?.includes("drizzle:PgInlineForeignKeys"),
    );
    ...
    if (fkSymbol && Array.isArray(table[fkSymbol])) {
      ...
      for (const [index, fk] of inlineForeignKeys.entries()) {
        ...
        if (fk && fk.reference && typeof fk.reference === "function") {
          try {
            ...
            if (
              referenceResult.columns &&
              Array.isArray(referenceResult.columns)
            ) {
              localColumns = referenceResult.columns.map((col: any) =>
                typeof col === "string"
                  ? col
                  : col.name || col.key || "unknown_column",
              );
            }

            if (
              referenceResult.foreignColumns &&
              Array.isArray(referenceResult.foreignColumns)
            ) {
            ...
            if (
              typeof referencedTableName === "object" &&
            ...
            logger.debug(
              `[INTROSPECTOR] Error processing foreign key reference:`,
              error,
            );
          }
        }
      ...
      logger.debug(
        `[INTROSPECTOR] No inline foreign keys found, trying fallback methods`,
      );
    }
  ...
  private extractReferencedTableName(reference: any): string | null {
    logger.debug(`[INTROSPECTOR] Extracting referenced table name from:`, {
      ...
      tableType:
        reference && reference.table ? typeof reference.table : undefined,
    ...
    if (reference.table && reference.table._ && reference.table._.name) {
      logger.debug(
        `[INTROSPECTOR] Found table name via table._.name: ${reference.table._.name}`,
      );
    ...
    if (reference.table) {
      ...
      for (const symbol of symbols) {
        if (symbol.description && symbol.description.includes("drizzle:Name")) {
          ...
          if (typeof tableName === "string") {
            logger.debug(
              `[INTROSPECTOR] Found table name via symbol: ${tableName}`,
            );
            return tableName;
          }
        }
    ...
    if (reference.name && typeof reference.name === "string") {
      logger.debug(
        `[INTROSPECTOR] Found table name via name property: ${reference.name}`,
      );
    ...
    if (typeof reference === "function") {
      try {
        ...
        if (referencedColumn && referencedColumn.table) {
          return this.extractReferencedTableName({
            table: referencedColumn.table,
          });
        }
      } catch (error) {
    ...
    if (reference.table) {
      ...
      if (table.tableName) {
        logger.debug(
          `[INTROSPECTOR] Found table name via tableName: ${table.tableName}`,
        );
      ...
      if (table.dbName) {
        logger.debug(
          `[INTROSPECTOR] Found table name via dbName: ${table.dbName}`,
        );
        return table.dbName;
      }
  ...
  private parseIndexes(table: any): IndexDefinition[] {
    ...
    if (tableConfig && tableConfig.extraConfigBuilder) {
      logger.debug(
        `[INTROSPECTOR] Found extraConfigBuilder, attempting to extract constraints`,
      );
      try {
        ...
        if (Array.isArray(extraConfig)) {
          logger.debug(
            `[INTROSPECTOR] ExtraConfig has ${extraConfig.length} items`,
          );
          for (const item of extraConfig) {
            ...
            if (item && item._ && item._.unique) {
              ...
              const columnNames =
                item._.columns?.map((col: any) => col.name) || [];
              logger.debug(
        ...
        logger.debug(
          `[INTROSPECTOR] Could not parse extra config for table constraints:`,
          error,
        );
      }
    }
    ...
    if (indexes.length === 0) {
      logger.debug(
        `[INTROSPECTOR] Still no constraints found, trying pattern-based extraction`,
      );
      ...
      if (tableName.includes("base_entities")) {
        ...
        logger.debug(
          `[INTROSPECTOR] Added pattern-based unique constraint for base_entities`,
        );
    ...
    logger.debug(
      `[INTROSPECTOR] Found ${indexes.length} indexes/constraints:`,
      indexes,
    );
    return indexes;
  }
  ...
  private parseCheckConstraints(
    ...
    logger.debug(
      `[INTROSPECTOR] Parsing check constraints. Has table._:`,
      !!tableConfig,
    );
    ...
    if (tableConfig && tableConfig.extraConfigBuilder) {
      try {
        ...
        if (Array.isArray(extraConfig)) {
          for (const item of extraConfig) {
            if (item && item._ && item._.type === "check") {
              ...
              logger.debug(
                `[INTROSPECTOR] Found check constraint: ${item._.name}`,
              );
            }
          }
        }
        ...
        logger.debug(
          `[INTROSPECTOR] Could not parse check constraints:`,
          error,
        );
      }
    }
    ...
    if (tableName.includes("dependent_entities")) {
      ...
      logger.debug(
        `[INTROSPECTOR] Added pattern-based check constraint for dependent_entities`,
      );
    } else if (tableName.includes("complex_relations")) {
      checkConstraints.push({
        name: "strength_range",
      ...
      logger.debug(
        `[INTROSPECTOR] Added pattern-based check constraint for complex_relations`,
      );
    }
  ...
  private parseCompositePrimaryKey(
    table: any,
  ): { name: string; columns: string[] } | undefined {
    ...
    if (tableConfig && tableConfig.extraConfigBuilder) {
      try {
        ...
        if (Array.isArray(extraConfig)) {
          for (const item of extraConfig) {
            if (
              ...
              item._ &&
              item._.name &&
              item._.type === "PrimaryKeyBuilder"
            ) {
              // Extract column names from the primary key definition
              const columnNames =
                item._.columns?.map((col: any) => col.name || col) || [];
              logger.debug(
        ...
        logger.debug(
          `[INTROSPECTOR] Could not parse composite primary key:`,
          error,
        );
      }
    }
  ...
  private mapDrizzleColumnType(
    ...
    if (
      ...
      const dimensions =
        config.dimensions || config.customTypeParams?.dimensions || 384;
      return `vector(${dimensions})`;
    }

    // Handle numberTimestamp specifically
    if (
      config.sqlName?.includes("numberTimestamp") ||
      columnType === "numberTimestamp"
    ) {
  ...
  private formatDefaultValue(defaultValue: any): string | undefined {
    ...
    if (defaultValue && typeof defaultValue === "object") {
      ...
      if (
        ...
        defaultValue.constructor.name === "Object"
      ) {
      ...
      if (defaultValue.constructor && defaultValue.constructor.name === "SQL") {
        ...
        if (
          sqlStr.includes("gen_random_uuid()") ||
          sqlStr.includes("GEN_RANDOM_UUID()")
        ) {
    ...
    logger.debug(
      `[INTROSPECTOR] Could not format default value, returning undefined`,
    );
    return undefined;
  }
  ...
  generateCreateTableSQL(
    ...
    const uniqueConstraints = tableDef.indexes
      ...
      .map(
        (idx) =>
          `CONSTRAINT "${idx.name}" UNIQUE ("${idx.columns.join('", "')}")`,
      );
...
export class PluginNamespaceManager {
  ...
  async getPluginSchema(pluginName: string): Promise<string> {
    if (pluginName === "@elizaos/plugin-sql") {
      ...
      try {
        ...
        logger.debug(
          "Could not determine search_path, defaulting to public schema.",
        );
      }
      return "public";
    }
  ...
  async createTable(
    ...
    const createTableSQL = introspector.generateCreateTableSQL(
      tableDef,
      schemaName,
    );
  ...
  async addConstraints(
    ...
    if (tableDef.foreignKeys.length > 0) {
      const introspector = new DrizzleSchemaIntrospector();
      const constraintSQLs = introspector.generateForeignKeySQL(
        tableDef,
        schemaName,
      );
      for (let i = 0; i < tableDef.foreignKeys.length; i++) {
        ...
        try {
          ...
          logger.debug(
            `[CUSTOM MIGRATOR] Successfully added foreign key constraint: ${fk.name}`,
          );
        } catch (error: any) {
          ...
          if (errorMessage.includes("already exists")) {
            logger.debug(
              `[CUSTOM MIGRATOR] Foreign key constraint already exists: ${fk.name}`,
            );
          } else {
            logger.warn(
  ...
  async installRequiredExtensions(requiredExtensions: string[]): Promise<void> {
    for (const extension of requiredExtensions) {
      try {
        ...
        logger.warn(
          `Could not install extension ${extension}: ${errorDetails.message}`,
        );
...
export async function runPluginMigrations(
  ...
  logger.debug(
    `[CUSTOM MIGRATOR] Starting migration for plugin: ${pluginName}`,
  );
  ...
  try {
    ...
    const errorDetails = extractErrorDetails(error);
    logger.error(
      `[CUSTOM MIGRATOR] Database connection failed: ${errorDetails.message}`,
    );
    if (errorDetails.stack) {
  ...
  const existingTables =
    await namespaceManager.introspectExistingTables(schemaName);
  ...
  try {
    ...
    for (const tableName of sortedTableNames) {
      ...
      logger.debug(
        `[CUSTOM MIGRATOR] Table ${tableDef.name} exists: ${tableExists}`,
      );
      ...
      if (!tableExists) {
        ...
        try {
          ...
          if (errorDetails.stack) {
            logger.error(
              `[CUSTOM MIGRATOR] Table creation stack trace: ${errorDetails.stack}`,
            );
          }
          throw new Error(
            `Failed to create table ${tableDef.name}: ${errorDetails.message}`,
          );
        }
      } else {
        logger.debug(
          `[CUSTOM MIGRATOR] Table ${tableDef.name} already exists, skipping creation`,
        );
      }
    }
    ...
    for (const tableName of sortedTableNames) {
      ...
      if (
        tableDef.foreignKeys.length > 0 ||
        tableDef.checkConstraints.length > 0
      ) {
        logger.debug(
          `[CUSTOM MIGRATOR] Adding constraints for table: ${tableDef.name}`,
          {
    ...
    logger.debug(
      `[CUSTOM MIGRATOR] Completed migration for plugin: ${pluginName}`,
    );
  } catch (error) {
    const errorDetails = extractErrorDetails(error);
    logger.error(
    ...
    if (errorDetails.stack) {
      logger.error(
        `[CUSTOM MIGRATOR] Migration stack trace: ${errorDetails.stack}`,
      );
    }
    throw new Error(
      `Migration failed for plugin ${pluginName}: ${errorDetails.message}`,
    );
  }
}
