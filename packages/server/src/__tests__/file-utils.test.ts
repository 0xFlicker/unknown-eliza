...
describe("File Utilities", () => {
  ...
  describe("createSecureUploadDir", () => {
    ...
    it("should reject IDs with path traversal attempts", () => {
      expect(() =>
        createSecureUploadDir("../../../etc/passwd", "agents"),
      ).toThrow("Invalid agent ID: contains illegal characters");

      expect(() =>
        createSecureUploadDir("test/../../passwd", "channels"),
      ).toThrow("Invalid channel ID: contains illegal characters");
    });
  ...
  describe("sanitizeFilename", () => {
    ...
    it("should remove special characters", () => {
      expect(sanitizeFilename("test:file*name?.jpg")).toBe(
        "test_file_name_.jpg",
      );
  ...
  describe("cleanupFile", () => {
    ...
    it("should normalize paths before checking", () => {
      ...
      expect(fsMock.unlinkSync).toHaveBeenCalledWith(
        path.resolve("/test/app/uploads/test.jpg"),
      );
    });
  });
  ...
  describe("cleanupFiles", () => {
    it("should cleanup multiple files", () => {
      ...
      expect(fsMock.unlinkSync).toHaveBeenCalledWith(
        path.resolve(files[0].tempFilePath),
      );
      expect(fsMock.unlinkSync).toHaveBeenCalledWith(
        path.resolve(files[1].tempFilePath),
      );
      expect(fsMock.unlinkSync).toHaveBeenCalledWith(
        path.resolve(files[2].tempFilePath),
      );
    });
