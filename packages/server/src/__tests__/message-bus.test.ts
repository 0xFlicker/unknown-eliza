...
describe("MessageBusService", () => {
  ...
  beforeEach(async () => {
    ...
    mockRuntime.ensureWorldExists = jest
      .fn()
      .mockReturnValue(Promise.resolve(undefined));
    mockRuntime.ensureRoomExists = jest
      .fn()
      .mockReturnValue(Promise.resolve(undefined));
    mockRuntime.getEntityById = jest
      .fn()
      .mockReturnValue(Promise.resolve(null));
    mockRuntime.createEntity = jest
      .fn()
      .mockReturnValue(Promise.resolve(undefined));
    mockRuntime.getMemoryById = jest
      .fn()
      .mockReturnValue(Promise.resolve(null));
    mockRuntime.createMemory = jest
      .fn()
      .mockReturnValue(Promise.resolve("mem-123"));
    ...
    mockFetch.mockImplementation((url) => {
      ...
      if (
        url.includes("/api/messaging/central-servers/") &&
      ...
      if (
        url.includes("/api/messaging/central-channels/") &&
      ...
      if (
        url.includes(`/api/messaging/agents/${mockRuntime.agentId}/servers`)
      ) {
        return Promise.resolve({
          ok: true,
          json: async () => ({
