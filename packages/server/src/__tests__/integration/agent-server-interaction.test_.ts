...
describe("Agent-Server Interaction Integration Tests", () => {
  ...
  describe("Agent Registration and Management", () => {
    ...
    it("should unregister an agent", async () => {
      ...
      const initialCount = initialAgents.filter(
        (id) => id === agent1.agentId,
      ).length;
      ...
      const finalCount = finalAgents.filter(
        (id) => id === agent1.agentId,
      ).length;
    ...
    it("should handle invalid agent registration gracefully", async () => {
      ...
      await expect(agentServer.registerAgent({} as any)).rejects.toThrow(
        "Runtime missing agentId",
      );

      await expect(
        agentServer.registerAgent({ agentId: "test-id" } as any),
  ...
  describe("Channel Management", () => {
    ...
    it("should find or create DM channel", async () => {
      ...
      const channel1 = await agentServer.findOrCreateCentralDmChannel(
        user1Id,
        user2Id,
        serverId,
      );
      ...
      const channel2 = await agentServer.findOrCreateCentralDmChannel(
        user1Id,
        user2Id,
        serverId,
      );
      ...
      const channel3 = await agentServer.findOrCreateCentralDmChannel(
        user2Id,
        user1Id,
        serverId,
      );
  ...
  describe("Agent-Server Association", () => {
    ...
    it("should handle adding agent to non-existent server", async () => {
      ...
      await expect(
        agentServer.addAgentToServer(fakeServerId, agentId),
