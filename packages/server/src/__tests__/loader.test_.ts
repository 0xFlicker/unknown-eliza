...
describe("Loader Functions", () => {
  ...
  describe("loadCharactersFromUrl", () => {
    it("should load single character from URL", async () => {
      ...
      const result = await loadCharactersFromUrl(
        "https://example.com/character.json",
      );
    ...
    it("should throw error for HTTP error response", async () => {
      ...
      await expect(
        loadCharactersFromUrl("https://example.com/character.json"),
      ).rejects.toThrow("Failed to load character from URL");
    });
    ...
    it("should throw error for invalid JSON response", async () => {
      ...
      await expect(
        loadCharactersFromUrl("https://example.com/character.json"),
      ).rejects.toThrow("Invalid JSON response from URL");
    });
    ...
    it("should throw error for network failures", async () => {
      ...
      await expect(
        loadCharactersFromUrl("https://example.com/character.json"),
      ).rejects.toThrow("Failed to fetch character from URL");
    });
  });
  ...
  describe("loadCharacterTryPath", () => {
    it("should load character from URL", async () => {
      ...
      const result = await loadCharacterTryPath(
        "https://example.com/character.json",
      );
    ...
    it("should throw specific error for URL failures", async () => {
      ...
      await expect(
        loadCharacterTryPath("http://fail.com/character.json"),
      ).rejects.toThrow("Failed to load character from URL");
    });
  });
  ...
  describe("loadCharacters", () => {
    ...
    it("should load characters from storage when enabled", async () => {
      ...
      (fs.promises.readdir as any).mockReturnValue(
        Promise.resolve(["storage-char.json"]),
      );
      (fs.readFileSync as any).mockImplementation((path: string) => {
    ...
    it("should load remote characters when local paths fail", async () => {
      ...
      expect(logger.error).toHaveBeenCalledWith(
        expect.stringContaining(
          "Failed to load character from 'non-existent.json'",
        ),
      );
    ...
    it("should handle storage read errors gracefully", async () => {
      ...
      (fs.promises.mkdir as any).mockRejectedValue(
    ...
    it("should trim whitespace from comma-separated paths", async () => {
      ...
      expect(fs.readFileSync).toHaveBeenCalledWith(
        expect.stringContaining("char1.json"),
        "utf8",
      );
      expect(fs.readFileSync).toHaveBeenCalledWith(
        expect.stringContaining("char2.json"),
        "utf8",
      );
    });
  });
