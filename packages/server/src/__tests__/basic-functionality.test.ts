...
describe("Basic Server Functionality", () => {
  describe("Path utilities", () => {
    it("should handle path expansion logic", () => {
      ...
      expect(testExpandTildePath("~/test")).toBe(
  ...
  describe("UUID validation logic", () => {
    it("should validate UUID format correctly", () => {
      ...
      // Valid UUIDs
      expect(validateUuidPattern("123e4567-e89b-12d3-a456-426614174000")).toBe(
        true,
      );
      expect(validateUuidPattern("00000000-0000-0000-0000-000000000000")).toBe(
        true,
      );
      expect(validateUuidPattern("ffffffff-ffff-ffff-ffff-ffffffffffff")).toBe(
        true,
      );

      // Invalid UUIDs
      expect(validateUuidPattern("invalid-uuid")).toBe(false);
      expect(validateUuidPattern("123e4567e89b12d3a456426614174000")).toBe(
        false,
      ); // no dashes
      expect(validateUuidPattern("123e4567-e89b-12d3-a456-42661417400")).toBe(
        false,
      ); // too short
  ...
  describe("Security pattern detection", () => {
    it("should detect suspicious patterns", () => {
      ...
      // Clean inputs
      expect(
        detectSuspiciousPatterns("123e4567-e89b-12d3-a456-426614174000"),
      ).toBe(false);
    ...
    it("should detect script injection patterns", () => {
      ...
      expect(containsScriptInjection("<img src=x onerror=alert(1)>")).toBe(
        true,
      );
  ...
  describe("UI Enable/Disable Logic", () => {
    ...
    it("should generate appropriate log messages", () => {
      ...
      expect(getLogMessage(false, true)).toBe(
        "Web UI disabled for security (production mode)",
      );
      expect(getLogMessage(false, false)).toBe(
        "Web UI disabled for security (production mode)",
      );
    });
