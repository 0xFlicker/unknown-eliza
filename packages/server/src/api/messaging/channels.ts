...
export function createChannelsRouter(
  ...
  (router as any).post(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      const isValidServerId =
        server_id === DEFAULT_SERVER_ID || validateUuid(server_id);
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          error:
            "Missing required fields: channelId, server_id, author_id, content",
        });
      }
      ...
      try {
        ...
        try {
          const existingChannel =
            await serverInstance.getChannelDetails(channelIdParam);
          channelExists = !!existingChannel;
          logger.info(
            `[Messages Router] Channel ${channelIdParam} exists: ${channelExists}`,
          );
        } catch (error: unknown) {
          const errorMessage =
          ...
          try {
            ...
            if (isDmChannel) {
              ...
              const otherParticipant =
                metadata?.targetUserId || metadata?.recipientId;
        ...
        const createdRootMessage =
          await serverInstance.createMessage(newRootMessageData);
        ...
        logger.debug(
          `[Messages Router] Emitting new_message for channel ${channelIdParam}, message ${messageForBus.id}`,
        );
        ...
        if (serverInstance.socketIO) {
          logger.debug(
            `[Messages Router] Broadcasting messageBroadcast to socketIO for channel ${channelIdParam}, message ${messageForBus.id}`,
          );
  ...
  (router as any).get(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      if (!channelId) {
        return res
          .status(400)
          .json({ success: false, error: "Invalid channelId" });
      }
  ...
  (router as any).get(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      if (!serverId) {
        return res
          .status(400)
          .json({ success: false, error: "Invalid serverId" });
      }
      try {
      ...
      try {
        if (providedDmServerId) {
          ...
          const existingServer =
            await serverInstance.getServerById(providedDmServerId); // Assumes AgentServer has getServerById
          if (existingServer) {
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      try {
        const channelDetails =
          await serverInstance.getChannelDetails(channelId);
        if (!channelDetails) {
          return res
            .status(404)
        ...
        res
          .status(500)
          .json({ success: false, error: "Failed to fetch channel details" });
      }
    },
  );
  ...
  (router as any).get(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      try {
        const participants =
          await serverInstance.getChannelParticipants(channelId);
  ...
  (router as any).post(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      try {
        ...
        logger.info(
          `[Messages Router] Added agent ${agentId} to channel ${channelId}`,
        );
  ...
  (router as any).delete(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      try {
        ...
        logger.info(
          `[Messages Router] Removed agent ${agentId} from channel ${channelId}`,
        );
  ...
  (router as any).delete(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      if (!channelId || !messageId) {
        return res
          .status(400)
          .json({ success: false, error: "Invalid channelId or messageId" });
      }
      try {
        // First, delete the message from central database
        await serverInstance.deleteMessage(messageId);
        logger.info(
          `[Messages Router] Deleted message ${messageId} from central database`,
        );
  ...
  (router as any).delete(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Failed to clear messages" });
      }
    },
  );
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      try {
        ...
        if (
          ...
          res
            .status(400)
            .json({ success: false, error: "Invalid filename detected" });
          return;
        }
...
Respond with just the title, nothing else.
        ...
        if (!newTitle || newTitle.trim().length === 0) {
          logger.warn(
            `[ChatTitleEvaluator] Failed to generate title for room ${channelId}`,
          );
          return;
        }
        ...
        logger.info(
          `[ChatTitleEvaluator] Generated title: "${cleanTitle}" for room ${channelId}`,
        );
        ...
        logger.success(
          `[CHANNEL SUMMARIZE] Successfully summarized channel ${channelId}`,
        );
