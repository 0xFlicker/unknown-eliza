 ...
 */
export function createMessagingCoreRouter(
  ...
  (router as any).post(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      try {
        ...
        res
          .status(500)
          .json({ success: false, error: "Failed to submit agent message" });
      }
    },
  );
  ...
  (router as any).post(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      try {
        ...
        res
          .status(200)
          .json({ success: true, message: "Completion event emitted" });
      } catch (error) {
        logger.error(
  ...
  (router as any).post(
    ...
    async (req: express.Request, res: express.Response) => {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ success: false, error: "Invalid external message payload" });
      }
      ...
      try {
        ...
        const createdRootMessage =
          await serverInstance.createMessage(messageToCreate);
        ...
        if (serverInstance.socketIO) {
          serverInstance.socketIO
            .to(messageForBus.channel_id)
        ...
        res
          .status(500)
          .json({ success: false, error: "Failed to ingest message" });
      }
    },
  );
