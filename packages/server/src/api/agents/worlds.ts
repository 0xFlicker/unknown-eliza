...
export function createAgentWorldsRouter(
    ...
    try {
      ...
      if (!runtime) {
        return sendError(
          ...
          404,
          "NOT_FOUND",
          "No active agents found to get worlds",
        );
      }
  ...
  const createWorldHelper = async (
    ...
    try {
      ...
      const world = (await runtime.getAllWorlds()).find(
        (w) => w.id === worldId,
      );
  ...
  router.patch("/:agentId/worlds/:worldId", async (req, res) => {
    ...
    if (!agentId || !worldId) {
      return sendError(
        ...
        400,
        "INVALID_ID",
        "Invalid agent ID or world ID format",
      );
    }
    ...
    try {
      ...
      const refreshedWorld = (await runtime.getAllWorlds()).find(
        (w) => w.id === worldId,
      );
