...
function validateSecureFilePath(filePath: string): string {
  ...
  const projectUpload = path.resolve(
    process.cwd(),
    ".eliza",
    "data",
    "uploads",
  ...
  if (
    !resolvedPath.startsWith(systemTemp) &&
    !resolvedPath.startsWith(projectUpload)
  ) {
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        ...
        logger.info(
          `[AUDIO MESSAGE] Transcription for agent ${agentId}: ${transcription}`,
        );
        cleanupUploadedFile(audioFile);
        sendSuccess(res, {
          transcription,
