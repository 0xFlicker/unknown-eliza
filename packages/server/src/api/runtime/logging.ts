    ...
    try {
      ...
      const requestedLevelValue =
        requestedLevel === "all"
          ...
          : LOG_LEVELS[requestedLevel as keyof typeof LOG_LEVELS] ||
            LOG_LEVELS.info;
      ...
      const agentNamePopulationRate =
        totalLogs > 0 ? logsWithAgentNames / totalLogs : 0;
      const agentIdPopulationRate =
        totalLogs > 0 ? logsWithAgentIds / totalLogs : 0;
      ...
      logger.debug("Logs request processed", {
        ...
        logsWithAgentNames,
        logsWithAgentIds,
        agentNamePopulationRate:
          Math.round(agentNamePopulationRate * 100) + "%",
        ...
        uniqueAgentNamesInLogs: [
          ...new Set(recentLogs.map((log) => log.agentName)),
        ].filter(Boolean),
        exactAgentNameMatches: recentLogs.filter(
          (log) => log.agentName === requestedAgentName,
        ).length,
      });
