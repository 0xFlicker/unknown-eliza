      ...
      fn: async (runtime) => {
        try {
          ...
          if (contact.entityId !== entityId) {
            throw new Error(
              `Contact ID mismatch: expected ${entityId}, got ${contact.entityId}`,
            );
          }

          if (
            !contact.categories.includes("friend") ||
            !contact.categories.includes("colleague")
          ) {
    ...
    },
    {
      ...
      fn: async (runtime) => {
        try {
          ...
          const colleagues = await rolodexService.searchContacts({
          ...
          const foundColleague = colleagues.find(
            (c) => c.entityId === entityId2,
          );
          if (!foundColleague) {
