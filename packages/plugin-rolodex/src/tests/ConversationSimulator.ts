...
export class ConversationSimulator {
  ...
  private async processMessage(message: Memory): Promise<void> {
    try {
      ...
      logger.debug('[ConversationSimulator] Message processed through evaluators', {
  ...
  async cleanup(): Promise<void> {
    ...
    for (const [name, user] of this.users) {
      try {
        ...
        logger.warn(`[ConversationSimulator] Failed to cleanup user ${name}:`, error);
      }
    }
    ...
    for (const [name, room] of this.rooms) {
      try {
        ...
        logger.warn(`[ConversationSimulator] Failed to cleanup room ${name}:`, error);
      }
    }
